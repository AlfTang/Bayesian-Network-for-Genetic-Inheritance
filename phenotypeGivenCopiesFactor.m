...
I = prod(phenotypeFactor.card);
phenotypeFactor.val = zeros(1, I);
% Replace the zeros in phentoypeFactor.val with the correct values.

assignment = IndexToAssignment(1:I, phenotypeFactor.card);

for(i=1:I)
    if(assignment(i,1) == 1)
        phenotypeFactor.val(i) = alphaList(allelesToGenotypes(assignment(i,2),assignment(i,3)));
    else if(assignment(i,1) == 2)
	     phenotypeFactor.val(i) = 1 - alphaList(allelesToGenotypes(assignment(i,2),assignment(i,3)));
	 else  
	     printf("phenotype value is %d, wrong number!!",assignment(i,1));
    end;
end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end