...
genotypeFactor.card = numAlleles*(numAlleles+1)/2*ones(1,3);

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
p = zeros(1,genotypeFactor.card(1));

% Replace the zeros in genotypeFactor.val with the correct values.

Assignment = IndexToAssignment(1:prod(genotypeFactor.card(2:3)),genotypeFactor.card(2:3));

for(i=1:genotypeFactor.card(2)*genotypeFactor.card(3))
    p(:) = 0;
    fatherAllels = genotypesToAlleles(Assignment(i,1),:);
    motherAllels = genotypesToAlleles(Assignment(i,2),:);

    for(j=1:2)
        for(k=1:2)
            p(allelesToGenotypes(fatherAllels(j), motherAllels(k))) += 1;
        end;
    end;
    p = p./4;
    genotypeFactor.val((i-1)*genotypeFactor.card(1)+1:i*genotypeFactor.card(1)) = p;
end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end