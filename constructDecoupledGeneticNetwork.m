...
for(i=1:numPeople)
    if(pedigree.parents(i,1) == 0 && pedigree.parents(i,2) == 0)
        factorList(i) = actorList(i+numPeople) = childCopyGivenFreqsFactor(alleleFreqs, i);
        factorList(i+numPeople) = actorList(i+numPeople) = childCopyGivenFreqsFactor(alleleFreqs, i+numPeople);
    else 
